...
In addition, the HUD can show a button that allow the user to close the HUD. the button will be shown only if ```CloseMessage``` text is provided.
...
#### ```CancelEventKey```
The event key that the client will emit to after the HUD's cancel button was pressed (mandatory if ```CloseMessage``` is provided). 
#### ```Interval```
...
```json
{
    ...
    "Data": {
        ...
        "CloseMessage": "Press to close", //optional
